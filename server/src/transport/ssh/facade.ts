...
/**
 * The host string the SSH transport should connect to for a client: the
 * admin-chosen `sshTarget` override (#406) when set, otherwise the raw
 * `hostname`. This is the single source of the resolution rule — the transport
 * ({@link targetFromClient}) and the admin-facing effective-target display both
 * call it, so what the dashboard *shows* can never drift from what it *dials*.
 */
export function sshHostForClient(
  client: Pick<ClientRow, "hostname"> & { sshTarget?: string | null },
): string {
  return client.sshTarget ?? client.hostname;
}
