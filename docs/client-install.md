...
```bash
# Piped straight from the dashboard (pass args after `-s --`)
...
The server URL, enrolment token, and supervised user come from these flags
(or the `PCT_SERVER_URL` / `PCT_ENROLMENT_TOKEN` / `PCT_SUPERVISED_USERS`
environment variables) — the script does not prompt for them, so a run
without them aborts with a clear "missing --server-url / --enrolment-token"
message.
